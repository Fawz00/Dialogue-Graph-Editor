...
from PyQt6.QtWidgets import (QGraphicsItem, QStyleOptionGraphicsItem, QWidget, QHBoxLayout)
...
class BaseNode(QGraphicsItem):
    ...
    def change_socket(self, socket: SocketItem, is_exec: bool = True, data_type: DataType | None = None, label: str = "", prop_reference_path: list[str] = []) -> SocketItem:
    ...
    def add_socket_from_var(self, prop_var_path: list[str]) -> SocketItem | None:
        # Makesure variable is valid from properties
        var = None
        for path in prop_var_path:
            var = self.properties.get(path)
    ...
    def add_inline_input(self, socket: SocketItem, prop_var_name: str):
    ...
    def get_properties(self) -> dict[str, Variable]:
